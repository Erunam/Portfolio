	...
	double precision af,tf,a0(2),t0(2)
...
c	(1.0-q+par(3)*q)/(q+par(4)*(1.0-q)
	...
	t(2) = t(2)+eps*S2*(fi+(par(9)/par(10))*(par(8)-t(2)))*dx(2)
		...
		if (czas .gt. tauP) then
					write(11,1000)tau(1),a(1),t(1)
					write(12,1000)tau(2),a(2),t(2)
		
			if(eps*W1 .eq. 1) then
					ap = ((1-par(3))/S1)*a(1)+((1-par(4))/S2)*a(2)
					tp = ((1-par(3))/S1)*t(1)+((1-par(4))/S2)*t(2)
					czas = tau(1)
					write(13,1000)czas,ap,tp
			else
					a0(m) = a(m)
					t0(m) = t(m)
...
1002	format(f16.12,4x,f16.12,4x,f16.12,4x,f16.12)
	end
